...
)
{
    ...
    else
    {
        ...
        {
            g_pDebugControl->Output(DEBUG_OUTPUT_DEBUGGEE,
                "Unable to recognize the callback %s. "
                "Looks like you are not sure how to use this. Try checking with \"help\".\n", args
            );
            g_helpMenu = 0;
        }
        else
        {
            g_pDebugControl->Output(DEBUG_OUTPUT_DEBUGGEE, "Unable to recognize the callback %s.\n", args);
            g_helpMenu++;
        }
        return S_FALSE;
    }
